...
\begin{itemize}
  ...
  \begin{itemize}
    \item 特定機能病院
    \item 一般病床200床以上の地域医療支援病院
    \item 一般病床200床以上の紹介受診重点医療機関
  \end{itemize}
  \vspace{1em}
  \item 選定療養費額
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 再診・総外来への効果は限定的（選定療養費額やスイッチングコスト）
    \item 政策効果の持続性が薄かったと考えられる
  \end{itemize}
  \vspace{2em}
  \item 政策的含意
  \begin{itemize}
    \item 分析結果からは価格メカニズムによる誘導には限界があると言える
...
\begin{figure}[h] % 図の環境を開始
        \centering     % 中央揃え
        % width=0.8\textwidth でスライド横幅の80%の大きさに指定
        \includegraphics[width=0.5\textwidth]{figures/figure3-a_event_study_first.png}
\end{figure}
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item 今後の課題（修正分析案）
  \begin{itemize}
    \item \textbf{IPW-DID}：共変量による重み付けによって観察可能な属性の違いに起因するバイアスを調整した上でDIDを実施
    \item \textbf{Synthetic DID}：処置前期間のアウトカムパスを一致させるような重み付けを用いてDIDを実施
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{figure}[h] % 図の環境を開始
        \centering     % 中央揃え
        % width=0.8\textwidth でスライド横幅の80%の大きさに指定
        \includegraphics[width=0.7\textwidth]{figures/figure2-a_trend_first.png}
\end{figure}
\end{frame}
...
\begin{figure}[h] % 図の環境を開始
        \centering     % 中央揃え
        % width=0.8\textwidth でスライド横幅の80%の大きさに指定
        \includegraphics[width=0.7\textwidth]{figures/figure2-b_trend_return.png}
\end{figure}
\end{frame}
...
\begin{figure}[h] % 図の環境を開始
        \centering     % 中央揃え
        % width=0.8\textwidth でスライド横幅の80%の大きさに指定
        \includegraphics[width=0.7\textwidth]{figures/figure2-c_trend_total.png}
\end{figure}
\end{frame}
